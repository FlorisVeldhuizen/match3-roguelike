  ...
  useEffect(
    () =>
      subscribeGameEvents((event) => {
        switch (event.kind) {
          case 'damage-taken':
            if (event.amount > 0) setAssertive((p) => bump(p, `Took ${event.amount} damage`))
            else if (event.blocked > 0) setPolite((p) => bump(p, `Blocked ${event.blocked}`))
            return
          case 'healed':
          ...
          case 'phase-changed':
            ...
            else if (event.phase === 'game-over') setAssertive((p) => bump(p, 'Defeated'))
            return
